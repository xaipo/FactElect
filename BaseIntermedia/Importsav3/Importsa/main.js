...
var job = new CronJob({
    ...
    cronTime: '00 23 19 * * 1-7',//ejecuta siempre
    onTick: function () {
...
var complete_validation = function()
{
    var lastProccessed = log.readLast();
    var sql = Sqls.sqlCountRows(lastProccessed);
    var rowsProcess = DataBase.executeSql(sql);
    rowsProcess = rowsProcess[0][0].value;
    while (rowsProcess > 0) {
        process_individual(lastProccessed);

        lastProccessed = log.readLast();

        console.log('last Proccessed',lastProccessed);
        sql = Sqls.sqlCountRows(lastProccessed);
        
        rowsProcess = DataBase.executeSql(sql);
        rowsProcess = rowsProcess[0][0].value;
    }
}

var process_individual = function (id_last_factura) {
    lastProccessed = id_last_factura;
    //var lastProccessed=log.readLast();
    var sql2 = Sqls.sqlTopRows(lastProccessed);
    var id = undefined;
    id = DataBase.executeSql(sql2);
    //require('deasync').loopWhile(function(){return id === undefined});
    id = id[0];

    var sqlGetFactura = Sqls.sqlRead(id[0].value);
    
    var facturaDB = undefined;
    facturaDB = DataBase.executeSql(sqlGetFactura);
    //require('deasync').loopWhile(function(){return facturaDB === undefined});
    var n = facturaDB.length;
    var cantidadProd = facturaProcess.cantArti(facturaDB);//da la cantidad de productos
    var descuento;
    for (var i = 0; i < n; i++) {
        // var subtotal=facturaProcess.loadSubtotal(facturaDB[i]);
        descuento = facturaProcess.loadDescuento(facturaDB[i], cantidadProd);//da el valor de lso descuentos
    ...
    var subtotal;
    for (var i = 0; i < n; i++) {
        subtotal = facturaProcess.loadSubtotal(facturaDB[i]);//da el valor del subtotal
        if (subtotal !== undefined) {
            break;
        }
        //    var descuento=facturaProcess.loadDescuento(facturaDB[i,cantidadProd]);
    }
    ...
    for (var i = 0; i < n; i++) {
        ...
        facturaActual = facturaProcess.processFacturaSql(facturaDB[i], facturaActual, n, i, subtotal, descuento);

    }
    log.writeLast(facturaActual.informacion.id_documento);
}

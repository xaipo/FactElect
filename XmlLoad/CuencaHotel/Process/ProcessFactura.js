...
var forma = require('../Models/FormasPago');
...
module.exports = {
            ...
            if (i === 0) {
                ...
                if (vec[i].codcli !== '9999999999999') {
                    ...
                    switch (cantidad) {
                        ...
                        case 10:
                            factura.tipo_identifiacion_comprador.value = identificaciones.cedula;
                            break;
                        case 13:
                            factura.tipo_identifiacion_comprador.value = identificaciones.ruc;
                            break;
                        default:
                            factura.tipo_identifiacion_comprador.value = identificaciones.pasaporte;
                            break;
                    }
                }
    ...
    processFacturaAccess: function (factura, facturaActual, actual) {

        if (actual == 0) {

            /////////////// CABECERA PRINCIPAL ////////////////////////////

            facturaActual.secuencial.value = factura.ordenNumeroFactura;
            var dateAux = factura.ordenFecha.split('T');
            facturaActual.fecha_emision.value = dateAux[0];
            var identificacion = factura.clienteCI;
            identificacion = identificacion.toString();
            if (identificacion !== '9999999999999') {

                var cantidad = identificacion.length;

                switch (cantidad) {

                    case 10:
                        facturaActual.tipo_identifiacion_comprador.value = identificaciones.cedula;
                        break;
                    case 13:
                        facturaActual.tipo_identifiacion_comprador.value = identificaciones.ruc;
                        break;
                    default:
                        facturaActual.tipo_identifiacion_comprador.value = identificaciones.pasaporte;
                        break;
                }
            }
            facturaActual.identificacion_comprador.value = identificacion;
            facturaActual.direccion_comprador.value = factura.clienteDireccion;
            facturaActual.total_sin_impuestos.value = factura.subtotal;
            var porcentajePropina= new Big(log.readPropina());
            var porcentaje=porcentajePropina.div(100);
            //porcentaje=porcentaje.plus(1);
            var propina= porcentaje.times(facturaActual.total_sin_impuestos.value);
            facturaActual.propina.value=propina.toFixed(2).toString();
            /*
            if (factura.itemDescuentoValor != null) {
                facturaActual.descuento.value = parseFloat(factura.itemDescuentoValor);
            }
            */
            ...
            var impuestoTotal = new Impuesto();
            impuestoTotal.base_imponible.value = factura.subtotal;
            impuestoTotal.valor.value = factura.cantidadImpuesto;
            impuestoTotal.tarifa.value = porcentaje.doce;
            facturaActual.total_con_impuestos.value.push(impuestoTotal);
            facturaActual.importe_total.value = factura.totalAPagar;

            /////////////// PAGO PRINCIPAL ////////////////////////////
            var pago = new Pago();

            pago.plazo.value = 0;
            pago.total.value = factura.totalAPagar;

            var formaDePago = factura.formaDePago;
            if (formaDePago == 1) {
                pago.forma_pago.value = forma.sin_utilizacion_financiero;
            }
            else if (formaDePago == 2) {
                pago.forma_pago.value = forma.otros_con_financiero;
            }
            else {
                pago.forma_pago.value = forma.tarjeta_credito;
            }
            facturaActual.pagos.value.push(pago);

            /////////////// INFO ADICIONAL ////////////////////////////

            var email = new Adicionales();
            email.adicional.nombre = "email";
            if (factura.clienteEmail !== null) {
                email.adicional.value = factura.clienteEmail;
            }
            else {
                email.adicional.value = "cambiar_por_email_real@gmail.com";
            }
            facturaActual.info_adicional.value.push(email);


            if (factura.clienteTelefono !== null) {
                var telefono = new Adicionales();
                telefono.adicional.value = factura.clienteTelefono;
                telefono.adicional.nombre = "telefono";
                facturaActual.info_adicional.value.push(telefono);
            }
        }

        var detalle = new Detalle();
        detalle.codigo_principal.value = factura.itemId;
        detalle.descripcion.value = factura.itemNombre;
        detalle.cantidad.value = factura.itemCantidad;
        detalle.precio_unitario.value = factura.itemPrecioUnitario;
        if (factura.itemDescuentoValor !== null) {
            detalle.descuento.value = factura.itemDescuentoValor;
            facturaActual.total_descuento.value += factura.itemDescuentoValor;
        }
        detalle.precio_total_sin_impuesto.value = factura.itemTotalAplicadoDescuento;

        var impuestoDetalle = new Impuesto();
        impuestoDetalle.base_imponible.value = detalle.precio_total_sin_impuesto.value;
        impuestoDetalle.valor.value = impuestoDetalle.base_imponible.value * porcentajeIva.doce / 100.0;

        detalle.impuestos.value.push(impuestoDetalle);

...
app.controller('CargasController', ['$scope', '$http', 'myProvider', function ($scope, $http, myProvider) {
    ...
    $http({
        ...
        $scope.propina=parseInt(response.data);
     //  console.log($scope.propina);
    }, function errorCallback(response) {
        console.log('entra');
        //  Console.log(response);
        //$scope.mesaje = response.mensaje;
    ...
    $scope.setPropina= function (){

        $http({

            method: 'POST',
            url: myProvider.setPropina(),
            data:{
                propina: $scope.propina
            },
            headers: {
                'Content-Type': 'application/json'
            }
